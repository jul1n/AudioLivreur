/**
 * Studio Synthèse Vocale Hybride HD (Client-Side) - v0.5.1
 * - Distinction acoustique stricte Homme/Femme (PitchMod & RateMod) sur toutes les langues (Japonais, Chinois, etc.)
 */
...
class EdgeTtsClient {
    ...
    _speakWebSpeechDirect(text, options = {}) {
        ...
        const pitchMod = voiceMeta ? voiceMeta.PitchMod : (targetGender === "Male" ? 0.78 : 1.22);
        ...
        if (systemVoices.length > 0) {
            ...
            if (!matched && langVoices.length > 0) {
                if (targetGender === "Male") {
                    matched = langVoices.find(v => v.name.toLowerCase().includes("male") || v.name.toLowerCase().includes("homme") || v.name.toLowerCase().includes("keita") || v.name.toLowerCase().includes("ichiro"));
                } else {
                    matched = langVoices.find(v => v.name.toLowerCase().includes("female") || v.name.toLowerCase().includes("femme") || v.name.toLowerCase().includes("haruka") || v.name.toLowerCase().includes("nanami"));
                }
            }
