/**
 * Studio Synthèse Vocale Hybride HD (Client-Side) - v0.5.2
 * - Resolution intégrale de la différenciation vocale internationale (Arabe, Chinois, Japonais, Coréen, etc.)
 */
...
class EdgeTtsClient {
    ...
    _speakWebSpeechDirect(text, options = {}) {
        ...
        const pitchMod = voiceMeta ? voiceMeta.PitchMod : (targetGender === "Male" ? 0.74 : 1.26);
        ...
        if (systemVoices.length > 0) {
            ...
            const langVoices = systemVoices.filter(v => v.lang.replace('_','-').toLowerCase().startsWith(baseLang));
            ...
            if (!matched && langVoices.length > 0) {
                if (targetGender === "Male") {
                    matched = langVoices.find(v => v.name.toLowerCase().includes("male") || v.name.toLowerCase().includes("homme") || v.name.toLowerCase().includes("keita") || v.name.toLowerCase().includes("yunxi") || v.name.toLowerCase().includes("shakiru") || v.name.toLowerCase().includes("hamed"));
                } else {
                    matched = langVoices.find(v => v.name.toLowerCase().includes("female") || v.name.toLowerCase().includes("femme") || v.name.toLowerCase().includes("haruka") || v.name.toLowerCase().includes("xiaoxiao") || v.name.toLowerCase().includes("salma") || v.name.toLowerCase().includes("zariyah"));
                }
            }

            if (!matched && langVoices.length > 0) matched = langVoices[0];
            if (!matched && systemVoices.length > 0) matched = systemVoices[0];
